...
struct __启动窗口 : 窗口
{
	...
	void 事件_创建完毕()
	{
		// 创建原生按钮并嵌入到 Sciter DOM 占位符 #btn-placeholder
		按钮::参数 csBtn;
		csBtn.标题 = L"白易原生按钮控件";
		按钮1.创建(csBtn, this);

		SciterDom dom = st.取文档模型();
		dom.嵌入原生控件("#btn-placeholder", 按钮1.窗口句柄);

		// 创建原生编辑框并嵌入到 Sciter DOM 占位符 #edit-placeholder
		编辑框::参数 csEdit;
		csEdit.内容 = L"这是白易原生编辑框";
		csEdit.边框 = 编辑框边框::单线边框式;
		编辑框1.创建(csEdit, this);
		dom.嵌入原生控件("#edit-placeholder", 编辑框1.窗口句柄);

		// 创建原生选择框并嵌入到 Sciter DOM 占位符 #check-placeholder
		选择框::参数 csCheck;
		csCheck.标题 = L"白易原生选择框（勾选测试）";
		csCheck.选中 = true;
		选择框1.创建(csCheck, this);
		dom.嵌入原生控件("#check-placeholder", 选择框1.窗口句柄);
	}
