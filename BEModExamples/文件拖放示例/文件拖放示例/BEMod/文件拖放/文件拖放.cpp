...
#define PROP_CALLBACK   L"BE_DragDrop_Callba文件拖放示例"
...
LRESULT CALLBACK _文件拖放_窗口回调(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
    ...
    文件拖放回调 callba文件拖放示例 = (文件拖放回调)GetPropW(hwnd, PROP_CALLBACK);
    ...
    if (iMsg == WM_DROPFILES) {
        ...
        if (callba文件拖放示例 && allFiles.len() > 0) {
            callba文件拖放示例(allFiles);
        }
        return 0;
    }
