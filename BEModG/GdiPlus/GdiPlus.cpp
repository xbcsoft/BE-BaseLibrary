...
struct 真彩图 {
	...
	static HandleBITMAP 提取掩码位图(const Bytes& imgData) {
		if (imgData.size == 0) return NULL;

		IStream* pStream = NULL;
		if (FAILED(CreateStreamOnHGlobal(NULL, TRUE, &pStream))) return NULL;

		ULONG written;
		pStream->Write(imgData.buf, imgData.size, &written);
		LARGE_INTEGER li = { 0 };
		pStream->Seek(li, STREAM_SEEK_SET, NULL);
		...
		for (int y = 0; y < height; ++y) {
			...
			for (int x = 0; x < width; ++x) {
				// 提取 Alpha 通道 (PixelFormat32bppARGB)
				unsigned char alpha = (unsigned char)((pSrcRow[x] >> 24) & 0xFF);

				// 如果 Alpha 小于阈值 (例如 128)，则认为是透明的
				// 在 Win32 掩码中：1 (白) 代表透明，0 (黑) 代表不透明
				if (alpha < 128) {
					pDstRow[x / 8] |= (0x80 >> (x % 8));
				}
			}
