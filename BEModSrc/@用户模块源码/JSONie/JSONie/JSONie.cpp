...
#pragma warning(pop)
	...
	void 删属性(const StrU8& 属性名, char 分隔符 = '.') {
		_Eval(L"delete " + _取属性路径(属性名, 分隔符) + L";");
	}
	...
	double 取属性数值(const StrU8& 属性名, char 分隔符 = '.') {
		return _ExecNumMethod(_取属性路径(属性名, 分隔符));
	}

	// 取属性
	StrU8 取属性(const StrU8& 属性名, char 分隔符 = '.', bool 格式化取出对象文本 = false) {
		StrW path = _取属性路径(属性名, 分隔符);
		return WtoU8(_ExecTextMethod(格式化取出对象文本 ? (L"JSON.stringify(" + path + L")") : path));
	}

	// 取属性对象
	JSONie 取属性对象(const StrU8& 属性名, char 分隔符 = '.') {
		return JSONie(m_runtime, m_context, _取属性路径(属性名, 分隔符));
	}
...
private:
	...
	StrW _取属性路径(const StrU8& 属性名, char 分隔符) {
		if (属性名.len() == 0) return m_basePath;

		const StrW fullPath = U8toW(属性名);
		const wchar_t separator = (wchar_t)(unsigned char)分隔符;
		const wchar_t* current = fullPath;
		StrW path = m_basePath;

		for (;;) {
			const wchar_t* next = wcschr(current, separator);
			StrW key = next ? StrW(current, next - current) : StrW(current);
			path += L"['" + _转义json单引号反斜杠(WtoU8(key)) + L"']";
			if (!next) break;
			current = next + 1;
		}
		return path;
