...
/**Get name of SciterUI window class.
 *
 * \return \b LPCWSTR, name of SciterUI window class.
 *         \b NULL if initialization of the engine failed, Direct2D or DirectWrite are not supported on the OS.
 *
 * Use this function if you wish to create unicode version of SciterUI.
...
/**Returns components of of SciterUI engine version vector. 
...
/**Notifies that SciterUI is about to download a referred resource.
 *
 ...
 * #SCN_LOAD_DATA::outDataSize members of SCN_LOAD_DATA. SciterUI does not
 ...
 * this resource has been completely downloaded. SciterUI will send this
 ...
 * have been completely downloaded. SciterUI will send this notification
 * asynchronously.
 **/
 ...
 *
 * \param[in] hwnd \b HWINDOW, SciterUI window handle.
 * \param[in] uri \b LPCWSTR, URI of the data requested by SciterUI.
 * \param[in] data \b LPBYTE, pointer to data buffer.
 * \param[in] dataLength \b UINT, length of the data in bytes.
 * \return \b SBOOL, TRUE if SciterUI accepts the data or \c FALSE if error occured
...
/**Load HTML file.
 *
 * \param[in] hWndSciter \b HWINDOW, SciterUI window handle.
...
/**Update pending changes in SciterUI window.
 *
 * \param[in] hwnd \b HWINDOW, SciterUI window handle.
 *
 **/
 ...
 *             On Desktop versions of the SciterUI this function does nothing so can be ommited.
 *
 **/
 ...
 **/
 ...
 /**
  * SciterUI "Primordial" Loader used by SCITER_APP_RUN
  */
...
/**Get current pixels-per-inch metrics of the SciterUI window
 *
 * \param[in] hWndSciter \b HWINDOW, SciterUI window handle.
...
/**Creates instance of SciterUI Engine on window controlled by DirectX
*
* \param[in] hwnd \b HWINDOW, window handle to create SciterUI on.
...
* Optionally allows to render parts of document (separate DOM elements) as layers
*
* \param[in] HWINDOW \b hwnd, window handle to create SciterUI on.
* \param[in] HELEMENT \b elementToRenderOrNull,  html element to render. If NULL then the engine renders whole document.
