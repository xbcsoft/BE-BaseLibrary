...
public:
	...
	bool 创建(const 参数& cs, 容器类* 父窗口 = NULL, HWND 父句柄 = NULL) {
		...
		_窗口风格 = _窗口风格 | BS_AUTOCHECKBOX | BS_NOTIFY;
	...
	void 选中_(bool isChecked) {
		选中 = isChecked;
		if (窗口句柄) SendMessageW(窗口句柄, BM_SETCHECK, isChecked ? BST_CHECKED : BST_UNCHECKED, 0);
	}
...
LRESULT 选择框::挂接消息(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
	case WM_PAINT: {
		...
		// 1. 让系统先画，系统现在只会画出未被 Validate 的小方框（复选框按钮）
